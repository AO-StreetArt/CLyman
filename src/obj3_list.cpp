...
//Not going to parse error code or message as we dont care about them coming in, only going out
    ...
    if (d.HasMember("objects")) {
      ...
      if (objs_val->IsArray()) {
        for (auto& itr : objs_val->GetArray()) {
          ...
          if (key_iter != itr.MemberEnd()) new_obj->set_key( key_iter->value.GetString() );

          rapidjson::Value::ConstMemberIterator name_iter = itr.FindMember("name");
          if (name_iter != itr.MemberEnd()) new_obj->set_name( name_iter->value.GetString() );

          rapidjson::Value::ConstMemberIterator scn_iter = itr.FindMember("scene");
          if (scn_iter != itr.MemberEnd()) new_obj->set_scene( scn_iter->value.GetString() );

          rapidjson::Value::ConstMemberIterator owner_iter = itr.FindMember("owner");
          if (owner_iter != itr.MemberEnd()) new_obj->set_owner( owner_iter->value.GetString() );

          rapidjson::Value::ConstMemberIterator type_iter = itr.FindMember("type");
          if (type_iter != itr.MemberEnd()) new_obj->set_type( type_iter->value.GetString() );
          ...
          if (translation_iter != itr.MemberEnd()) {
            ...
            for (auto& trans_itr : translation_val.GetArray()) {
              if (i == 0) {trans->set_x(trans_itr.GetDouble());}
              else if (i == 1) {trans->set_y(trans_itr.GetDouble());}
          ...
          if (erot_iter != itr.MemberEnd()) {
            ...
            for (auto& erot_itr : erot_val.GetArray()) {
              if (i == 0) {erot->set_x(erot_itr.GetDouble());}
              else if (i == 1) {erot->set_y(erot_itr.GetDouble());}
          ...
          if (qrot_iter != itr.MemberEnd()) {
            ...
            for (auto& qrot_itr : qrot_val.GetArray()) {
              if (i == 0) {qrot->set_w(qrot_itr.GetDouble());}
              else if (i == 1) {qrot->set_x(qrot_itr.GetDouble());}
          ...
          if (scale_iter != itr.MemberEnd()) {
            ...
            for (auto& scale_itr : scale_val.GetArray()) {
              if (i == 0) {scl->set_x(scale_itr.GetDouble());}
              else if (i == 1) {scl->set_y(scale_itr.GetDouble());}
...
std::string Obj3List::to_json() {
  ...
  for (unsigned int a=0;a<objects.size();a++) {
    ...
    if ( !(objects[a]->get_key().empty()) ) {
    	writer.Key("key");
    	writer.String( objects[a]->get_key().c_str(), (rapidjson::SizeType)objects[a]->get_key().length() );
    }

    if ( !(objects[a]->get_name().empty()) ) {
    	writer.Key("name");
    	writer.String( objects[a]->get_name().c_str(), (rapidjson::SizeType)objects[a]->get_name().length() );
    }

    if ( !(objects[a]->get_scene().empty()) ) {
    	writer.Key("scene");
    	writer.String( objects[a]->get_scene().c_str(), (rapidjson::SizeType)objects[a]->get_scene().length() );
    }

    if ( !(objects[a]->get_type().empty()) ) {
    	writer.Key("type");
    	writer.String( objects[a]->get_type().c_str(), (rapidjson::SizeType)objects[a]->get_type().length() );
    }

    if ( !(objects[a]->get_subtype().empty()) ) {
    	writer.Key("subtype");
    	writer.String( objects[a]->get_subtype().c_str(), (rapidjson::SizeType)objects[a]->get_subtype().length() );
    }
...
std::string Obj3List::to_protobuf() {
  ...
  for (unsigned int i = 0; i < objects.size(); i++) {
    ...
    if ( !(objects[i]->get_key().empty()) ) new_obj->set_key(objects[i]->get_key());
    if ( !(objects[i]->get_name().empty()) ) new_obj->set_name(objects[i]->get_name());
    if ( !(objects[i]->get_scene().empty()) ) new_obj->set_scene_id(objects[i]->get_scene());
    if ( !(objects[i]->get_type().empty()) ) new_obj->set_type(objects[i]->get_type());
