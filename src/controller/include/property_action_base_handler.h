...
#ifndef SRC_CONTROLLER_INCLUDE_PROPERTY_ACTION_BASE_HANDLER_H_
#define SRC_CONTROLLER_INCLUDE_PROPERTY_ACTION_BASE_HANDLER_H_

class PropertyActionBaseRequestHandler: public ActionBaseRequestHandler<PropertyFrame>, public Poco::Net::HTTPRequestHandler {
 public:
  PropertyActionBaseRequestHandler(AOSSL::KeyValueStoreInterface *conf, DatabaseManagerInterface *db, \
      EventStreamPublisher *pub, ClusterManager *cluster, int mtype, std::string obj_id, std::string action_name) : \
      ActionBaseRequestHandler<PropertyFrame>(conf, db, pub, cluster, mtype, obj_id, action_name) {}
  PropertyActionBaseRequestHandler(AOSSL::KeyValueStoreInterface *conf, DatabaseManagerInterface *db, \
      EventStreamPublisher *pub, ClusterManager *cluster, int mtype, std::string id) : \
      ActionBaseRequestHandler<PropertyFrame>(conf, db, pub, cluster, mtype, id) {}
  void handleRequest(Poco::Net::HTTPServerRequest& request, Poco::Net::HTTPServerResponse& response) {
    ...
    if (doc.HasParseError()) {
      ...
      AnimationAction<PropertyFrame> *inp_doc = nullptr;
      try {
        inp_doc = json_factory.build_property_action(doc);
      } catch (std::exception& e) {
        logger.error("Exception encountered building Property Action");
        logger.error(e.what());
        response_body->set_error_code(TRANSLATION_ERROR);
      ...
      if (inp_doc) {
        ...
        if (msg_type == PROP_ACTION_CRT || msg_type == PROP_ACTION_UPD) {
...
#endif  // SRC_CONTROLLER_INCLUDE_PROPERTY_ACTION_BASE_HANDLER_H_
