...
#ifndef SRC_CONTROLLER_INCLUDE_OBJECT_FRAME_BASE_HANDLER_H_
#define SRC_CONTROLLER_INCLUDE_OBJECT_FRAME_BASE_HANDLER_H_

class ObjectFrameBaseRequestHandler: public FrameBaseRequestHandler<ObjectFrame>, public Poco::Net::HTTPRequestHandler {
 public:
  ObjectFrameBaseRequestHandler(AOSSL::KeyValueStoreInterface *conf, DatabaseManagerInterface *db, \
      EventStreamPublisher *pub, ClusterManager *cluster, int mtype, std::string obj_id, \
      std::string action_name) : FrameBaseRequestHandler<ObjectFrame>(conf, db, pub, cluster, mtype, obj_id, action_name) {}
  ObjectFrameBaseRequestHandler(AOSSL::KeyValueStoreInterface *conf, DatabaseManagerInterface *db, \
      EventStreamPublisher *pub, ClusterManager *cluster, int mtype, std::string id, std::string action_name, \
      std::string frame_ind) : FrameBaseRequestHandler<ObjectFrame>(conf, db, pub, cluster, mtype, id, action_name, frame_ind) {}
  void handleRequest(Poco::Net::HTTPServerRequest& request, Poco::Net::HTTPServerResponse& response) {
    ...
    if (doc.HasParseError() || !(frame_indx_parse_success)) {
      ...
      ObjectFrame *inp_doc = nullptr;
      try {
        inp_doc = json_factory.build_object_frame(doc);
      } catch (std::exception& e) {
      ...
      if (inp_doc) {
        ...
        if (msg_type == OBJ_FRAME_CRT || msg_type == OBJ_FRAME_UPD) {
...
#endif  // SRC_CONTROLLER_INCLUDE_OBJECT_FRAME_BASE_HANDLER_H_
