...
TEST_CASE( "Test Database Manager", "[integration]" ) {
  ...
  std::string owner2 = "vwxyz1234567";
    ...
    // Property Tests
  if (true) {
    PropertyInterface *prop1 = build_test_property(key1, name1, scene1, parent1, asset_sub_id1, 0, 1);
    PropertyInterface *prop2 = build_test_property(key2, name2, scene2, parent2, asset_sub_id2, 2, 3);
    prop1->add_value(1.0);
    prop2->add_value(2.0);
    prop2->add_value(3.0);
    std::string new_key;
